...
def listener(): 
    ...
    sub = rospy.Subscriber('/wheel_commands', WheelCommands, set_wheel_command_left) 
 
    #### CODE HERE ####
    # Add a Subscriber for the Right wheel
    # Subscribe to the "wheel_speed_desired_right" topic
#    subR = rospy.Subscriber('/wheel_command_right', WheelCommands, set_wheel_command_right) 
    #### END CODE ####
...
#    wheel_command_right = int(msg_in.commandR)
#    motors.motor2.setSpeed(wheel_command_right)
#    rospy.loginfo("right wheel set to {0}".format(wheel_command_right))    # could issue a Log message to ROS about it
