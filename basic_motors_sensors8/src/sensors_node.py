...
sensors_node.py
ROS Node to a continually read data from a Serial port (either '/dev/ttyS0' (Alamode) or '/dev/ttyUSB0' (Arduino Nano on USB) and publish it on a topic ("sensors_A0" or other)   
"""
...
# 
# Could also publish Encoders (E0 and E1, (in "counts")), more Analogs (A0-A5, in "levels" 0-1023) and Ultrasound (in microseconds till echo) readings
...
# Data comes in on the Serial port. Set that up and start it. 
    ...
    #----------setup serial--------------
    ser = serial.Serial('/dev/ttyS0')  #serial port to alamode is /dev/ttyS0. # port to Arduino Nano is /dev/ttyUSB0 
    ser.baudrate = 115200 
    ser.bytesize = 8
    ser.parity = 'N'
    ser.stopbits = 1
